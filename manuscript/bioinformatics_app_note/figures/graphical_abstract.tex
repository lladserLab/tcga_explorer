...
\node[stage title] at (12.30,8.02) {CONTRACT};
\draw[traceLine, line width=0.45pt] (11.83,7.70) -- (14.12,7.70);
\node[item, text width=2.15cm,
  font=\sffamily\fontsize{6.6}{7.7}\selectfont] at (11.90,7.43) {
  \textbf{Decision}\\[-1pt]
  \textcolor{traceMuted}{fixed $\rightarrow$ reported}\\[5pt]
  \textbf{Cohort}\\
  endpoint + sample rule\\
  $\rightarrow$ flow + exclusions\\[5pt]
  \textbf{Model}\\
  continuous primary\\
  $\rightarrow$ effect + sensitivities\\[5pt]
  \textbf{Family}\\
  grid frozen before run\\
  $\rightarrow$ ledger + adjusted $p$
};
\node[small, text width=2.15cm, fill=traceOliveLight, rounded corners=1pt, inner sep=4pt]
  at (11.90,2.63) {Null and discordant outputs remain visible.};
...
\node[item, text width=2.68cm,
  ...
  Effects + diagnostic plots\\
  Continuous/grouped CSVs\\
  Specification curve + ledger\\
  PH, RMST, EPV + Firth\\
  Methods + participant inputs\\
