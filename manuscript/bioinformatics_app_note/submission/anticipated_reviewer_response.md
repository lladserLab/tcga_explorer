...
| Missing dominant prior art | Addressed without a winner-takes-all feature table. Supplementary Table S1 positions the 12 closest resources by established strength and residual comparison point; the surrounding text covers PrognoScan, OncoLnc, ESurv, GSCA, TCGEx, Survival Genie 2, CaPSSA and SurvBoard. It explicitly recognizes domains in which other tools are broader or methodologically stronger, separates marker association from predictive-model benchmarking, and states that no independent audit of every version was performed. | `docs/publication/comparator_matrix.md`; `manuscript/bioinformatics_app_note/supplementary.tex`. |
...
| Concordance with existing tools and literature is missing | Partly addressed with explicit evidentiary boundaries. A prespecified three-case cBioPortal PanCancer Atlas panel contains one direction/support match, one direction-only match and one jointly unsupported result; shared TCGA data make this cross-pipeline corroboration, not independent validation. The main cases add CDC20 workflow concordance, independent-cohort/qRT-PCR BUB1B--PINK1 agreement and a literature-aligned but heterogeneous BIRC5 pan-cancer pattern. BAP1/PRAME is retained as the sole non-confirmatory case. | `docs/publication/benchmark/external_concordance_panel/`; `docs/publication/external_concordance_kmplotter_ca9_kirc.md`; `manuscript/bioinformatics_app_note/main.tex`; `manuscript/bioinformatics_app_note/supplementary.tex`. |
